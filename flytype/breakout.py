...
two only meet in cli.py, which passes a decoded LEFT/RIGHT/HOLD action in and a
plain correct/incorrect/hold outcome out. The paddle moves only as far as the
decoded action says; no tick ever nudges it toward the ball.
...
y=12..64, and objects are sized so that at least a handful of photoreceptors
...
class BreakoutGame:
    """One brick-breaker episode driven by a two-choice action stream.
    ...
    def __init__(self, settings, rng):
        ...
        self.ball_vy_magnitude = (
            PADDLE_TOP - BALL_SIZE - self.wall_bottom
        ) / self.descent_ticks
    ...
    def _serve(self):
        ...
        """
        margin = BALL_SIZE + 4
        self.ball_x = margin + self.rng.random() * (WIDTH - 2 * margin - BALL_SIZE)
    ...
    def tracking_direction(self):
        ...
        """
        gap = (self.ball_x + BALL_SIZE / 2) - self.paddle_center
    ...
    def _hit_paddle(self):
        if self.ball_vy <= 0:
            return False
        bx0, bx1 = self.ball_x, self.ball_x + BALL_SIZE
        by1 = self.ball_y + BALL_SIZE
        if by1 < PADDLE_TOP or self.ball_y > PADDLE_TOP + PADDLE_HEIGHT:
            return False
        if bx1 <= self.paddle_x or bx0 >= self.paddle_x + self.paddle_width:
            return False
        ...
        if offset < 0:
            self.ball_vx = -self.ball_vx
        self.ball_vy = -self.ball_vy_magnitude
        self.ball_y = PADDLE_TOP - BALL_SIZE
        self.paddle_hits += 1
        return True
